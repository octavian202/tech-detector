...
function convertMatch(match, techName) {
  const { type, key } = parseSearch(match.search)
  let regex = null
  if (match.regexp) {
    regex = rubyRegexToJs(match.regexp)
  } else if (match.text) {
    const escaped = String(match.text).replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
    regex = escaped
  }
  if (!regex) return null

  if (type === 'headers') {
    return { name: techName, headers: [{ key, regex }] }
  }
  return { name: techName, html: [{ regex }] }
}
