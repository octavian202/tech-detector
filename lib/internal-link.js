...
/**
 * App / SaaS / docs surfaces (high value for tech fingerprinting).
 */
const APP_SEGMENT_PATH_RE =
  /\/(app|dashboard|signup|register|pricing|docs)(\/|$)/i
...
function sortByEcommercePriority(hrefs) {
  ...
  const scored = hrefs.map((href) => {
    ...
    const appSegment = APP_SEGMENT_PATH_RE.test(pathname) ? 1 : 0
    const firstSegment = pathname.split('/').filter(Boolean)[0] || ''
    const diverseBoost = firstSegment && !segmentSeen.has(firstSegment) ? 1 : 0
    if (firstSegment) {
      segmentSeen.add(firstSegment)
    }
    return { href, ecommerce, appSegment, diverseBoost }
  })
  ...
  scored.sort((a, b) => {
    ...
    if (b.appSegment !== a.appSegment) {
      return b.appSegment - a.appSegment
    }
    if (b.diverseBoost !== a.diverseBoost) {
      return b.diverseBoost - a.diverseBoost
    }
