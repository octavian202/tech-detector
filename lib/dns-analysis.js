...
async function analyzeDns(hostname) {
  ...
  try {
    ...
    for (const target of cname) {
      ...
      if (t.includes('cdn.cloudflare.net') || t.includes('cloudflare')) {
      ...
      if (t.includes('vercel-dns.com') || t.includes('vercel.app')) {
        if (!seen.has('Vercel')) {
          seen.add('Vercel')
          results.push({
            name: 'Vercel',
